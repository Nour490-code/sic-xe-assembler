...
public class ObjectCodeGenerator  {
    ...
    private File Out_WithObjectCode = new File(_OutFileName);
 ...
 //   private File Out_WithObjectCode = new File(_OutFileName);
    ...
    public String CalculateTA (String Pc,String Base ,String Ta) {
        ...
        if (Ta != null) {
            ...
            if (Displacment > 127 || Displacment < -128) {
                Displacment = TA - BASE;
                System.out.println("Base relative addressing used with displacement: " + Displacment + "  Base : "+ Base);
    ...
    public String GetForm3ObjectCode(String [] Instruction, String PC )
    {
        short  n ;
        short i ;
        char x;
        ...
        if (Menimonic.equals("LDB")) {
            String address ;
            if (AddressingMode != 1)
                 address = Symbols.get(Operand.substring(1));
            else
                 address = Symbols.get(Operand);

            if (address != null) {
                Base = address;  // Store the value in Base for future Base Relative addressing
                System.out.println("Base register set to: " + Base);
            } else {
                System.out.println("Error: Symbol not found for LDB: " + Operand);
            }
        }
        if (Menimonic.equals("BASE")) {
            Base = Symbols.get(Operand);
            System.out.println("Base directive set base address to: " + Base);
        }
...
//                System.out.println(Base);
            ...
            case 2:
                if (Character.isLetter(Operand.charAt(1))) {
                    ...
                    if (Integer.parseInt(Displacement,16) > 127 || Integer.parseInt(Displacement,16) < -128) {
                        b = '1';
                        p = '0';
                    }
                } else {
    ...
    public String GetForm4ObjectCode(String [] Instruction , String PC  )
    {
        short  n = 1 ;
        short i = 1;
        char x;
    ...
    public void GenerateObjectCode() {
        ...
        for (int i = 1; i < _InFileData.size(); i++) {
            ...
            // Check if it's a WORD or BYTE directive
            if (Menimonic.equals("WORD")) {
                String value = parts[parts.length - 1];
                // Convert the value to hex and ensure it is 6 digits (24 bits)
                String objectCode = String.format("%06X", Integer.parseInt(value));
                AddObjectCodeToLine(objectCode, i);


            } else if (Menimonic.equals("BYTE")) {
                String value = parts[parts.length - 1];
                String objectCode = "";

                if (value.startsWith("C'") && value.endsWith("'")) {
                    // Character representation
                    String chars = value.substring(2, value.length() - 1);
                    ...
                    // Hex representation
                    objectCode = value.substring(2, value.length() - 1);
                ...
                switch (InstructionFormatNumber) {
                    ...
                    case 2:
                        ...
                        break;
                    case 3:
                        AddObjectCodeToLine(GetForm3ObjectCode(parts, Integer.toString(PC, 16)), i);
                        //System.out.println(_InFileData.get(i));
                        break;
                    case 4:
                        AddObjectCodeToLine(GetForm4ObjectCode(parts, Integer.toString(PC, 16)), i);
    ...
    {
        if (Out_WithObjectCode.exists())
        {
            InputFileLien line = new InputFileLien();
            String OutputLine;
            for (int i = 0; i < _InFileData.size(); i++) {
                OutputLine = _InFileData.get(i) ;
               // System.out.println(_InFileData.get(i));
                Output.println(OutputLine);
               // System.out.println(_InFileData);
            }
            Output.close();
        }
    }
